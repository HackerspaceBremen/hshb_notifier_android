...
public class DashClockExtensionService extends DashClockExtension {
	...
	protected void onUpdateData(final int arg0) {
		...
		final DataEvent statusEvent = (DataEvent) EventBus.getDefault()
				.getStickyEvent(DataEvent.class);
		if (statusEvent != null && statusEvent.getData() != null) {
			final ExtensionData data = new ExtensionData().visible(true);
			...
			if (statusEvent.getData().isSpaceOpen()) {
				final String title = this.getString(R.string.space_open,
						timeString);
				final String simpleTitle = this
						.getString(R.string.space_open_simple);
				data.status(simpleTitle).expandedTitle(title)
						.icon(R.drawable.hshb_widget_icon);
			} else {
				final String title = this.getString(R.string.space_closed,
						timeString);
				final String simpleTitle = this
						.getString(R.string.space_closed_simple);
				data.status(simpleTitle).expandedTitle(title)
						.icon(R.drawable.hshb_widget_icon);
			}

			if (statusEvent.getData().getMessage() != null
					&& statusEvent.getData().getMessage().length() > 0) {
			...
			widgetIntent
					.putExtra("status_json",
							SpaceDataJsonParser.parse(statusEvent.getData())
									.toString());
